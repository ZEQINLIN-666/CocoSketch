...
public:
    ...
    void HCOtherBench(uint32_t MEMORY, double alpha){
        ...
        for(uint32_t i = 1;i <= MAX_TRAFFIC;++i){
            ...
            tupleSketch[0] = new HCOtherSketch<TUPLES>(mem);
            tupleSketch[1] = new HCOtherSketch<TUPLES>(mem);

            for(uint32_t j = 0;j < i - 1;++j){
                sketch[j][0] = new HCOtherSketch<uint64_t>(mem);
                sketch[j][1] = new HCOtherSketch<uint64_t>(mem);
            }
            ...
            std::string saveFile = "HC-" + fileName + "-" + tupleSketch[0]->name + "-" + std::to_string(MEMORY) +
    ...
    void HCOurBench(uint32_t MEMORY, double alpha){
        MultiAbstract<TUPLES>* sketch[2];

        sketch[0] = new SimpleCU<TUPLES>(MEMORY);
        sketch[1] = new SimpleCU<TUPLES>(MEMORY);
        ...
        std::string saveFile = "HC-" + fileName + "-" + sketch[0]->name + "-" + std::to_string(MEMORY) +
...
private:
    ...
    void CompareHC(T mp, T record, COUNT_TYPE threshold, uint32_t key_type, std::ofstream& outfile){
        ...
        //outfile << "aae," << aae / bothHH << std::endl;
        //outfile << "are," << are / bothHH << std::endl;
        outfile << std::endl;
    }
